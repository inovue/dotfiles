...
```bash
./run.sh --print-prompt --preset logo -g 4 "Financial Fantasy Logos" -s flat \
...
```bash
./run.sh --confirm <TOKEN> --grill-ack <GRILL_ACK> --preset logo -g 4 "Financial Fantasy Logos" -s flat \
...
```bash
./run.sh --confirm <TOKEN> --preset logo -g 4 "Logos" --no-rembg --tight --items cells.json --out out/logos-chroma
```

Horizontal wordmarks (8 variants):

```bash
./run.sh --confirm <TOKEN> --preset wordmark -g 4x2 "Brand Wordmarks" --items cells-8.json --out out/wordmarks-v1
```

## Step 4 — Automated inspect (no API)

```bash
./run.sh --inspect out/logos-v1
```

Target: **≥ 75/100 (grade B)**. Inspect checks files, alpha fill, dimensions, seam detector.

```bash
./run.sh --inspect out/logos-v1 -j   # JSON for agents
```
