...
`--inspect <outDir>` computes this score from `manifest.json` + `sheet.grid.json`. **≥ 75 = production-ready for automated pipeline**; human review still required for brand fit. **`inspect` returns `ok: true` only when there are no error-level findings**, including score below 75 (`MIN_SHIP_SCORE`).
...
| **v2.3.0** | **~80** | strict grid/aspect input validation; duplicate cell id guard; rembg letterbox (no stretch-fill); `manifest.failed.json` on mid-pack failure |
| **v2.3.1** | **~81** | magenta/gray subject collision gates; genmedia non-retryable 422/auth/policy fail-fast |
| **v2.3.2** | **~81** | corrupt JSON handling in `--inspect`; score ≥75 enforced as `ok`; dual bypass warn |
| **v2.3.3** | **~81** | auth preflight before generation (`assertGenmediaAuthReady`); pipeline-level `manifest.failed.json` on rembg/seam/pack failures |
