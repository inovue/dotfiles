...
describe('batch inspect', () => {
  ...
  test('inspectBatch fails on corrupt manifest JSON', async () => {
    const outDir = path.resolve(process.cwd(), 'tests/fixtures/out/inspect-corrupt-manifest');
    if (fs.existsSync(outDir)) fs.rmSync(outDir, { recursive: true, force: true });
    fs.mkdirSync(outDir, { recursive: true });
    ...
    assert.ok(
      result.findings.some(
        (f) => f.level === 'error' && f.message.includes('manifest.json: invalid JSON'),
      ),
    );
  });

  test('inspectBatch fails on corrupt sheet.grid.json', async () => {
    const outDir = path.resolve(process.cwd(), 'tests/fixtures/out/inspect-corrupt-grid');
    if (fs.existsSync(outDir)) fs.rmSync(outDir, { recursive: true, force: true });
    await writeFixtureBatch(outDir);
    fs.writeFileSync(path.join(outDir, 'sheet.grid.json'), '[broken');

    const result = await inspectBatch(outDir);
    assert.strictEqual(result.ok, false);
    assert.ok(
      result.findings.some(
        (f) => f.level === 'error' && f.message.includes('sheet.grid.json: invalid JSON'),
      ),
    );
    assert.strictEqual(result.gridMeta, undefined);
  });

  test('inspectBatch warns when both bypass flags are set', async () => {
    const outDir = path.resolve(process.cwd(), 'tests/fixtures/out/inspect-dual-bypass');
    if (fs.existsSync(outDir)) fs.rmSync(outDir, { recursive: true, force: true });
    await writeFixtureBatch(outDir);

    const result = await inspectBatch(outDir, { allowWeakSeams: true, allowEmptyCells: true });
    assert.ok(
      result.findings.some(
        (f) => f.level === 'warn' && f.message.includes('allow-weak-seams') && f.message.includes('allow-empty-cells'),
      ),
    );
  });
