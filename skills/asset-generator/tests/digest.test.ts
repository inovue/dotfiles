...
describe('batch digest & confirm token', () => {
  ...
  test('digest binds existing ref paths by resolved absolute path', () => {
    const refDir = fs.mkdtempSync(path.join(os.tmpdir(), 'digest-ref-'));
    const refFile = path.join(refDir, 'logo.png');
    fs.writeFileSync(refFile, Buffer.from([0x89, 0x50, 0x4e, 0x47]));

    const relRef = path.join('.', path.basename(refDir), 'logo.png');
    const cwd = process.cwd();
    process.chdir(refDir.replace(/[/\\][^/\\]+$/, ''));

    try {
      const opts = { countOrGrid: '4', refImages: [relRef] };
      const rel = computeBatchDigest(4, 'Theme', cells, opts);
      const abs = computeBatchDigest(4, 'Theme', cells, { ...opts, refImages: [refFile] });
