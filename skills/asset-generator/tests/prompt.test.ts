...
describe('parseGridCount', () => {
  ...
  test('supports preset aliases', () => {
    assert.deepStrictEqual(parseGridCount('4'), { cols: 2, rows: 2, total: 4 });
    assert.deepStrictEqual(parseGridCount('single'), { cols: 1, rows: 1, total: 1 });
    assert.deepStrictEqual(parseGridCount(undefined), { cols: 1, rows: 1, total: 1 });
  });

  test('rejects invalid grid counts', () => {
    for (const bad of ['5', '2', '0x0', '100x1', '9x9']) {
      assert.throws(() => parseGridCount(bad), PipelineValidationError);
    }
  });
});
