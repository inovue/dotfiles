...
import { normalizeCellId } from './items.js';
...
export function formatCellFilename(id: string | undefined, label: string, index: number): string {
  if (id) {
    const clean = normalizeCellId(id);
    if (clean) {
...
export async function extractAndSaveAssets(
  ...
  const writeFailureMarker = (err: unknown): void => {
      ...
      JSON.stringify(
        {
          status: 'failed',
          error: message,
          partialItems: manifestItems.map((item) => ({
            id: item.id,
