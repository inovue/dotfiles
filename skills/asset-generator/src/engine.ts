...
import type { AssetManifest, GeneratorOptions, GridMeta } from './types.js';
import { buildPrompt } from './prompt.js';
import { ensureDirSync, generateBatchId, parseOutPath, resolveDigestOutDir } from './paths.js';
import { assertGenmediaAuthReady } from './genmedia.js';
import { writePipelineFailedMarker } from './fail-marker.js';
...
export async function runAssetGenerator(options: GeneratorOptions): Promise<AssetManifest> {
  ...
  if (options.tight && options.modelQuality === 'low' && options.preset !== 'logo' && options.preset !== 'wordmark') {
  ...
  if (options.allowWeakSeams && options.allowEmptyCells) {
    warn('Both seam and alpha quality gates are disabled (--allow-weak-seams and --allow-empty-cells).');
  }
  ...
  if (gridInfo.total > 1) {
    ...
    if (options.rawCell) log(`📐 Framing: Raw Cell Bounds`);
    else if (options.tight) log(`📐 Framing: Tight chroma-key trim`);
  ...
  try {
    ...
    gridMeta = await detectGridSeams(rawPath, gridInfo.cols, gridInfo.rows);
    validateGridSeams(gridMeta, { allowWeakSeams: options.allowWeakSeams });
    await validateNoMagentaSubjectCollision(rawPath, gridMeta);
    if (options.rembg === false || options.tight) {
      await validateChromaKeyLeavesSubject(rawPath, gridMeta);
    }
    ...
    stage = options.rembg !== false ? 'rembg' : 'chroma-key';
    ...
    writePipelineFailedMarker(outDir, err, { batchId, stage });
    throw err;
  }
