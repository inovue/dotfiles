...
export const MIN_SHIP_SCORE = 75;
...
function readJsonFile<T>(
  ...
  try {
    ...
    return { ok: false, error: `${label}: invalid JSON — ${detail}` };
  }
}
...
export async function inspectBatch(outDir: string, options: InspectOptions = {}): Promise<InspectResult> {
  ...
  const emptyBreakdown: QualityBreakdown = {
    seamDetection: 0,
    alphaCoverage: 0,
    dimensions: 0,
    visualHeuristics: 0,
  ...
  if (options.allowWeakSeams && options.allowEmptyCells) {
    findings.push({
      level: 'warn',
      message: 'Both --allow-weak-seams and --allow-empty-cells are set; seam and alpha gates are relaxed',
    });
  }
  ...
  const manifestResult = readJsonFile<AssetManifest>(manifestPath, 'manifest.json');
  if (!manifestResult.ok) {
    findings.push({ level: 'error', message: manifestResult.error });
    return {
      ok: false,
      outDir: resolved,
      findings,
  ...
  if (fs.existsSync(gridPath)) {
    const gridResult = readJsonFile<GridMeta>(gridPath, 'sheet.grid.json');
    if (gridResult.ok) {
      gridMeta = gridResult.data;
    } else {
      findings.push({ level: 'error', message: gridResult.error });
    }
  }
  ...
  if (breakdown.total < MIN_SHIP_SCORE) {
    findings.push({
      level: 'error',
      message: `Score ${breakdown.total}/100 below production threshold (${MIN_SHIP_SCORE}). See quality-rubric.md.`,
    });
  }
...
export function formatInspectReport(result: InspectResult): string {
  ...
  if (result.ok) {
    ...
    const belowShip = result.breakdown.total < MIN_SHIP_SCORE;
    if (belowShip) {
      lines.push(`\x1b[31m✗ FAIL — score ${result.breakdown.total}/100 below production threshold (${MIN_SHIP_SCORE})\x1b[0m`);
    } else {
